...
class ResConfigSettings(models.TransientModel):
	...
	szm_method_lotsn = fields.Selection([
		('std', 'Standard'),
		('cust', 'Custom'),
		('date', 'Date')],string='Lot/SN Method',related="company_id.szm_method_lotsn",readonly=False)


	@api.model
	def get_values(self):
		res = super(ResConfigSettings, self).get_values()
		res.update(szm_apply_method = self.env['ir.config_parameter'].sudo().get_param('szm_sn-lot_mod.szm_apply_method'))
		return res
